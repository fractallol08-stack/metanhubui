...
-- Truncate keybind text to fit within max width
...
-- Show settings panel with module options
...
function Library:create_ui()
    ...
    -- Settings Panel (right side panel for module options)
    ...
    local SettingsPanelDivider = Instance.new('Frame')
    SettingsPanelDivider.Name = 'Divider'
    SettingsPanelDivider.Size = UDim2.new(1, -20, 0, 1)
    SettingsPanelDivider.Position = UDim2.new(0.5, 0, 0, 50)
    SettingsPanelDivider.AnchorPoint = Vector2.new(0.5, 0)
    SettingsPanelDivider.BackgroundColor3 = Color3.fromRGB(52, 66, 89)
    SettingsPanelDivider.BackgroundTransparency = 0.5
    SettingsPanelDivider.BorderSizePixel = 0
    SettingsPanelDivider.Parent = SettingsPanel
        ...
        function TabManager:create_module(settings: any)
            ...
            function ModuleManager:scale_keybind(empty: boolean)
                if Library._config._keybinds[settings.flag] and not empty then
                    ...
                    keybind_string = Library:truncate_keybind_text(keybind_string)
                    ...
                    -- Limit max width
                    local width = math.min(font_size.X + 6, Library._max_keybind_width)
                    Keybind.Size = UDim2.fromOffset(width, 15)
                    Keybind.ClipsDescendants = true
                    TextLabel.Size = UDim2.fromOffset(width - 6, 13)
            ...
            function ModuleManager:create_colorpicker(settings)
                ...
                ColorPicker.Name = 'ColorPicker_' .. settings.flag
                ColorPicker.Size = UDim2.new(0, 207, 0, 22)
                ColorPicker.Parent = Options
                ColorPicker.LayoutOrder = LayoutOrderModule
                ...
                -- Color Preview Box (TextButton for click events)
                ...
                -- Saturation/Value Picker (2D area)
                ...
                function ColorPickerManager:toggle()
                    -- Close previous color picker if exists
                ...
                function ColorPickerManager:set_color(color)
                    ...
                    ColorPickerManager._hue, ColorPickerManager._saturation, ColorPickerManager._value = RGBtoHSV(color)
                ...
                Connections['colorpicker_close_'..settings.flag] = UserInputService.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and ColorPickerManager._open then
                        ...
                        -- Check if click is outside picker and preview
